...
```{r ans1}
pdf("zhirui_problem1.pdf", width=14, height=14)
# If the width and height are set too small, the color key in the PDF file cannot be displayed properly. 

heatmap.2(edata,
          main = "Heatmap for Problem 1", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
          col=my_palette,       # use on color palette defined earlier 
          dendrogram="column",  # for the option (b)
          scale = "column",     # for the option (c)
          Colv=TRUE, Rowv=TRUE,  # for the option (a)
          )
dev.off()
```
...
```{r ans2}
# Exploration
n_PCs <- ncol(PC)-ncol(pData(bottomly.eset))
for (i in 1:(n_PCs-1)) {
  for (j in (i+1):n_PCs) {
    p <- ggplot(PC) + 
      geom_point(aes_string(x=paste0("V", i), y=paste0("V", j), col="as.factor(strain)")) + 
      labs(x=paste0("PC", i), y=paste0("PC", j), color="Strain", 
           title=paste("Projection on PC", i, "vs PC", j))
    
    ggsave(filename=paste0("exploration_log/PC", i, "_PC", j, ".png"), 
           plot=p, width=6, height=5)
  }
}

# After exploration, we found that the combination of PC2 and PC3 could separate the strains well. 
p <- ggplot(PC) + geom_point(aes(x=V2, y=V3, col=as.factor(strain))) + 
  labs(x="PC2", y="PC3", color="Strain", title="Projection on PC2 vs PC3 for Problem 2")
ggsave(filename="zhirui_problem2.pdf", plot=p, width=6, height=5)
```
...
```{r ans3}
loadings <- data.table(svd.out$u)
p <- ggplot(loadings) + 
  geom_point(aes(x=V1, y=V2)) +
  labs(x="Left Singular Vector 1", y="Left Singular Vector 2",
       title="Scatter Plot of Top 2 Left Singular Vectors for Problem 3") 
ggsave(filename="zhirui_problem3.pdf", plot=p, width=6, height=6)
```
...
```{r ans4}
loadings_melt <- melt(loadings[, 1:5, with=FALSE], variable.name="PC", value.name="Loading")
p <- ggplot(loadings_melt) + 
  geom_violin(aes(x=PC, y=Loading),draw_quantiles = c(0.25, 0.5, 0.75)) + 
  # geom_jitter(aes(x=PC, y=Loading)) + 
  labs(title = "Violin Plots of Top 5 Left Singular Vectors for Problem 4") 
ggsave(filename="zhirui_problem4.pdf", plot=p, width=8, height=5)
```
...
```{r ans5}
set.seed(1)
rs_kmeans <- kmeans(edata, centers=5)
clusters <- as.factor(rs_kmeans$cluster)
tsne_out <- Rtsne(edata,pca=FALSE,perplexity=60)
tsne_out = data.table(tsne_out$Y)
tsne_out[, Cluster:=clusters]

p <- ggplot(tsne_out) +
  geom_point(aes(x=V1, y=V2, color=Cluster)) +
  labs(title="t-SNE Projection Colored by K-Means Clusters",
       x="t-SNE Dimension 1", y="t-SNE Dimension 2") + 
  scale_color_brewer(palette="Set1")

ggsave(filename="zhirui_problem5.pdf", plot=p, width=6, height=6)
```
