...
# If the command "install.packages("bladderbatch")" doesn's work due to the version of R, 
...
batch_summary <- as.data.table(pheno)[, .(
  Cancer_Cancer = sum(cancer=="Cancer", na.rm=TRUE),
  Cancer_Normal = sum(cancer=="Normal", na.rm=TRUE), 
  Cancer_Biopsy = sum(cancer=="Biopsy", na.rm=TRUE), 
  Outcome_Normal = sum(outcome=="Normal", na.rm=TRUE),
  Outcome_sTCC_CIS = sum(outcome=="sTCC+CIS", na.rm=TRUE),
  Outcome_mTCC = sum(outcome=="mTCC", na.rm=TRUE),
  Outcome_Biopsy = sum(outcome=="Biopsy", na.rm=TRUE)
), by=batch]

pdf("zhirui_problem1.pdf", width=16, height=3)
grid.table(batch_summary)
dev.off()
```

From the above tables, it can be seen that the variables of cancer and outcome are not evenly distributed across batches, which may introduce bias. 
...
```{r ans2}
sorted_indices <- order(pheno$batch)
edata_sorted <- edata[, sorted_indices]
combat_edata_sorted <- combat_edata[, sorted_indices]

pdf("zhirui_problem2.pdf", height=14, width=14)
heatmap.2(edata_sorted,
          main="Heatmap Before ComBat (Sorted by Batch)", # heat map title
          notecol="black", # change font color of cell labels to black 
          density.info="none", # turns off density plot inside color legend
          trace="none", # turns off trace lines inside the heat map 
          margins=c(12,9), # widens margins around plot
          col=my_palette, # use on color palette defined earlier 
          dendrogram="row",  # cluster the rows, but do not cluster the columns 
          scale="row")

heatmap.2(combat_edata_sorted,
          main="Heatmap After ComBat (Sorted by Batch)", # heat map title
          notecol="black", # change font color of cell labels to black 
          density.info="none", # turns off density plot inside color legend
          trace="none", # turns off trace lines inside the heat map 
          margins=c(12,9), # widens margins around plot
...
```{r ans3}
cor_matrix_before <- cor(edata_sorted, method="pearson")
cor_matrix_after <- cor(combat_edata_sorted, method="pearson")

pdf("zhirui_problem3.pdf", height=14, width=14)
heatmap.2(cor_matrix_before,
          main="Sample Correlation Heatmap Before ComBat", # heat map title
          notecol="black", # change font color of cell labels to black 
          density.info="none",  # turns off density plot inside color legend 
          trace="none", # turns off trace lines inside the heat map 
          margins=c(12,9), # widens margins around plot 
          col=my_palette, # use on color palette defined earlier  
          dendrogram="none", # only draw a row dendrogram 
          labRow=pheno$batch, # label rows by batch numbers
          labCol=pheno$cancer) # label columns by cancer vs. normal

heatmap.2(cor_matrix_after,
          main="Sample Correlation Heatmap After ComBat", # heat map title
          notecol="black", # change font color of cell labels to black 
          density.info="none", # turns off density plot inside color legend 
          trace="none", # turns off trace lines inside the heat map 
          margins=c(12,9), # widens margins around plot 
...
edata_bot <- t(scale(t(edata_bot), scale=FALSE, center=TRUE))
pheno_bot <- pData(bottomly.eset)

combat_edata_bot = ComBat(dat=edata_bot, batch=pheno_bot$experiment.number, mod=model.matrix(~1, data=pheno_bot), par.prior=TRUE, prior.plots=TRUE)


mod_uncorrected <- lm(t(edata_bot) ~ as.factor(pheno_bot$strain) + as.factor(pheno_bot$experiment.number))
mod_uncorrected_tidy <- tidy(mod_uncorrected)

mod_corrected <- lm(t(combat_edata_bot) ~ as.factor(pheno_bot$strain))
mod_corrected_tidy <- tidy(mod_corrected)

est_compare <- tibble(
  Uncorrected = mod_uncorrected_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("estimate") %>% unlist,
  Corrected = mod_corrected_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("estimate") %>% unlist)

pdf("zhirui_problem4.pdf", width=9, height=6)
ggplot(est_compare, aes(x=Uncorrected, y=Corrected)) + 
  geom_point(col="darkgrey", alpha=.5, size=.5) + 
  geom_abline(intercept=0, slope=1, col="darkred") + 
  geom_smooth(method = "lm", se = TRUE)  + 
  theme_bw()

ggplot(mod_corrected_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J"), aes(x=p.value)) + 
  geom_histogram(bins=100, fill="darkorange") +
  ggtitle("P-values After ComBat Correction")
...
```{r ans5}
mod_bot <- model.matrix(~as.factor(strain),data=pheno_bot)
mod0_bot <- model.matrix(~1, data=pheno_bot)
sva_output_bot <- sva(edata_bot, mod_bot, mod0_bot, n.sv=num.sv(edata_bot,mod_bot,method="leek"))

# Add the surrogate variables to the model matrix
mod_sva <- lm(t(edata_bot) ~ as.factor(pheno_bot$strain) + sva_output_bot$sv)
mod_sva_tidy <- tidy(mod_sva)

pdf("zhirui_problem5.pdf", width=9, height=6)
est_compare <- tibble(
  ComBat = mod_corrected_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("estimate") %>% unlist,
  SVA = mod_sva_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("estimate") %>% unlist)
...
pvalues <- tibble(
  ComBat = mod_corrected_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("p.value") %>% unlist,
  SVA = mod_sva_tidy %>% filter(term == "as.factor(pheno_bot$strain)DBA/2J") %>% select("p.value") %>% unlist)
